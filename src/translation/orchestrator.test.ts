...
describe('TranslationOrchestrator', () => {
  it('按到达顺序拼接单一缓冲区并返回完整校验结果', async () => {
    ...
    const saveResult = vi.fn<(result: TranslationResult, shouldCommit: () => boolean) => Promise<void>>()
  ...
  it('持久化收到最小提交条件，中止后条件失效', async () => {
    ...
    const saveResult = vi.fn<(result: TranslationResult, condition: () => boolean) => Promise<void>>()
      .mockImplementation(async (_result, condition) => { shouldCommit = condition })
